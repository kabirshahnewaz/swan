...
There is **no build system, package manager, test suite, or git repo**. Files are opened directly in a browser (e.g. `start swan_overview.html` on Windows). To "run" or preview, just open the file; for live reload while editing, serve the folder with any static server (`python -m http.server`).
...
- `swan_top_posts_report.html` is the exception — it loads Chart.js from a **CDN** (`cdnjs.cloudflare.com`), so it needs network access.
- `swan_overview.html` and `swan_reddit_roadmap.html` have **no charts**.
